...
def generate_invoice_PDF(invoice: Invoice, base_in):
    """Generate invoice PDF"""

    # Details
    student = invoice.lesson.student
    teacher = invoice.lesson.teacher
    instrument = invoice.lesson.instrument.name
    lesson_date = invoice.lesson.date
    price = invoice.price
    invoice_ref_id = str(student.id) + "-" + str(invoice.id)
    date = time.strftime("%d/%m/%Y")

    # PDF generation
    padding = 1.5 * cm
    height, width = A5
    base = base_in.Canvas(student.first_name + "_" +
                          student.last_name + "_Invoice_" +
    ...
    base.drawRightString(width-padding-(stringWidth(str("Ammount"), "Helvetica", 12)/2), 185-50, "£" + str(price))
